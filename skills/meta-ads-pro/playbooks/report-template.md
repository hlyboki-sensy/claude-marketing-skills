...
3. **Поріг значущості.** Не виносити вирок об'єкту на `<1000 показів` АБО `<3 конверсії` АБО `<2 дні` даних (`safety_rules.md` → «Минимум данных»). Менше — пиши «**недостатньо даних** — потрібна вибірка `breakdowns=...` / довший період», а не вигадуй.
